      ...
      if ord(ru) < 32 or ord(ru) == 127: # if control char
        case ord(ru):
          ...
          of 0: discard
            ...
            terminal.setStyle(stdout, {styleDim})
            stdout.write this.controllRune #ru #"¶"
            setColors(this.app, this.activeStyle[])
            cx += 1
...
proc onScroll(this:Controll, event:KMEvent)=
  case event.evType:
    of KMEventKind.ScrollUp: TextArea(this).onPgUp()
    of KMEventKind.ScrollDown: TextArea(this).onPgDown()
    else: discard
