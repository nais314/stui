...
proc tb2test(pbPtr: ptr TextBox){.gcsafe.}=
    while true and pbPTR[].app.flags[int(quitFlag)] < 1:
        pbPTR[].val = genId(5)
        pbPTR[].draw(true)
...
discard menuW.currentNode.addChild("Quit",proc() = appQuit()) # appQuit see stui_template
...
var
    statusTss = loadConfig(splitPath(currentSourcePath()).head & 
                            DirSep & "stui" & DirSep & "status.tss")
...
proc statusTss_test(win: ptr Window, app: ptr App, sty: ptr){.gcsafe.}=
    randomize()
    if win != nil and app != nil and sty != nil:
        while true and app.flags[int(quitFlag)] < 1:
            for i in 0 .. win.controlls.high :
                if win.controlls[i] of TextBox:
                    case rand(10):
                        of 0: 
                            win.controlls[i].styles["input"].fgColor = app.styles["input"].fgColor
                            win.controlls[i].styles["input"].bgColor = app.styles["input"].bgColor
                        of 1: 
                            win.controlls[i].styles["input"].fgColor = sty[]["error"].fgColor
                            win.controlls[i].styles["input"].bgColor = sty[]["error"].bgColor
                        of 2: 
                            win.controlls[i].styles["input"].fgColor = sty[]["warning"].fgColor
                            win.controlls[i].styles["input"].bgColor = sty[]["warning"].bgColor
                        of 3: 
                            win.controlls[i].styles["input"].fgColor = sty[]["info"].fgColor
                            win.controlls[i].styles["input"].bgColor = sty[]["info"].bgColor
                        of 4:
                            win.controlls[i].styles["input"].fgColor = sty[]["success"].fgColor
                            win.controlls[i].styles["input"].bgColor = sty[]["success"].bgColor
                        else:
                            win.controlls[i].styles["input"].fgColor = app.styles["input"].fgColor
                            win.controlls[i].styles["input"].bgColor = app.styles["input"].bgColor

                    ui_textbox.draw( TextBox(win.controlls[i]), true)
...
for i in 1 .. 50:
    if i mod 3 == 0:
        discard lg1.dataset.add(($i, (rand(30).float * -1)))
        #discard FloatDataset2D(lg1.dataset).add(($i,i.float * -1))
    else:
        discard lg1.dataset.add(($i,rand(30).float))
...
lg1.dataSet.conditionalStyler = lg1_conditionalFormat

var
    lineGraphTss = loadConfig(splitPath(currentSourcePath()).head & 
                                DirSep & "stui" & DirSep & "linegraph.tss")
...
for i in 1 .. 50:
    if i mod 3 == 0:
        discard lg2.dataset.add(($i, (rand(30).int * -1)))
        #discard FloatDataset2D(lg1.dataset).add(($i,i.float * -1))
    else:
        discard lg2.dataset.add(($i,rand(30).int))
        #discard FloatDataset2D(lg1.dataset).add(($i,i.float))
...
for i in 1 .. lg3MaxItems:
    discard lg3.dataset.add(($i,  rand(100).float))
    #[ if i mod 3 == 0:
...
#...........................
proc lg3sim(lg3Ptr: ptr LineGraph[float]) {.gcsafe.} =
    var 
        r:float
        c:int
    while true and lg3Ptr[].app.flags[int(quitFlag)] < 1:
        #lg3Ptr.dataset.values.add(("added from thread", 25.float))
        c += 1
        r = rand([5,10,20,30,40,50,60,70,80,100]).float
        discard lg3Ptr.dataset.add(("from thread [" & $c & "]", r))
        
        #[ if lg3Ptr.dataset.maxValue < r : 
            lg3Ptr.dataset.maxValue = r
        if lg3Ptr.dataset.minValue > r : 
            lg3Ptr.dataset.minValue = r ]#
