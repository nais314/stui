...
# done: add label to T Controll, + `label=` + if label=="" + labelHeight==0
...
#-------------------------------------------------------------------------------
# Basic App init
var app = newApp(splitPath(currentSourcePath()).head ) # (path to themes)acquire(app.termlock) # whatever im creating, i dont want output yet ;)
...
var
    statusTss = loadConfig("stui/status.tss")
...
var
    lineGraphTss = loadConfig("stui/linegraph.tss")
